 ...
 * @author CloudKing
 */
@Repository("backupStorageDAO")
public final class BackupStorageDAO extends BaseDAO<BackupStorageEntity> {
    ...
    public List<Object[]> query(String name, Long machineRoomID, Long machineRackID, PageInfo pageInfo)
        ...
        StringBuilder queryQL = new StringBuilder(
                        "select resource_,resource_.machineRack.name,resource_.machineRack.machineRoom.name  from BackupStorageEntity resource_ where 1=1");
        //条件拼装列表
    ...
    public Integer getQueryCount(String name, Long machineRoomID, Long machineRackID) throws SQLException {
        StringBuilder queryQL = new StringBuilder("select count(*) from BackupStorageEntity where 1=1");
        //条件拼装列表
        List<String> paramNames = new ArrayList<String>();
        List<Object> paramValues = new ArrayList<Object>();
        // 名字 
     ...
     *             sql异常
     */
    public BackupStorageEntity getByNameAndRack(String name, Long rackID) throws SQLException {
        return uniqueResult("from BackupStorageEntity where name=:name and machineRack.id=:rackID", Arrays
     ...
     * @param start
     * @param limit
     * @return List<Object[]> Object[0] 是BackupStorageEntity,Object[1] 是机架名,Object[2] 是机房名,
     * @throws SQLException
     *             sql异常
     */
    public List<Object[]> queryNoPage(String name, String type, Long machineRoomID, Long machineRackID)
                    throws SQLException {
        StringBuilder queryQL = new StringBuilder(
                        "select resource_,resourcePool_ from BackupStorageEntity resource_ left join resource_.storageResourcePool resourcePool_ where 1=1");
        //条件拼装列表
        List<String> paramNames = new ArrayList<String>();
        List<Object> paramValues = new ArrayList<Object>();
        // 名字 
    ...
    public List<Object[]> queryStorageWithDomain() throws SQLException {
        return list("select storageResource_,domain_ from BackupStorageEntity storageResource_ left join storageResource_.domains domain_");
    }
     ...
     *             sql异常
     */
    public List<BackupStorageEntity> queryStorageInSpecifyDomain(Long domainId) throws SQLException {
        return list(
                        "select storageResource_ from BackupStorageEntity storageResource_ inner join storageResource_.domain domain_ "
    ...
    public Integer getCountByMachineRack(Long machineRackID) throws SQLException {
        return Integer.parseInt(uniqueResultObject(
                        "select count(*) from BackupStorageEntity where machineRack.id=:rackID", "rackID",
     ...
     *             sql异常
     */
    public BackupStorageEntity findStorageByIp(String ip) throws SQLException {
        return (BackupStorageEntity) uniqueResult("from BackupStorageEntity where ip = :ip", Arrays
    ...
    public BackupStorageEntity getByCapacity(Long capacity) throws SQLException {
        List<BackupStorageEntity> tmpResults = pageQuery(" from BackupStorageEntity  "
        ...
        BackupStorageEntity result = null;
     ...
     *             sql异常
     */
    public BackupStorageEntity getMaxDiskStorage() throws SQLException {
        List<BackupStorageEntity> tmpResults = pageQuery("from BackupStorageEntity    order by availableCapacity DESC",
                        0, 1);
        BackupStorageEntity result = null;
        if (!tmpResults.isEmpty()) {
            result = tmpResults.get(0);
     ...
     *             sql
     */
    public Object[] sumBackupStorageChartByDomain(Long domainId) throws SQLException {
        return (Object[]) uniqueResultObject("select sum(tb_sr.capacity),sum(tb_sr.availableCapacity) "
                        + " from BackupStorageEntity tb_sr "
     ...
     *             sql
     */
    public Object[] sumBackupStorageChartById(Long storageId) throws SQLException {
        return (Object[]) uniqueResultObject("select sum(tb_sr.capacity),sum(tb_sr.availableCapacity) "
                        + " from BackupStorageEntity tb_sr where tb_sr.id=:storageId", Arrays
                        .asList(new String[] { "storageId" }), Arrays.asList(new Object[] { storageId }));
    }
     ...
     *             sql
     */
    public Object[] sumBackupStorageChartInSpecifyDomains(String domainIds) throws SQLException {
     ...
     *             sql
     */
    public List<BackupStorageEntity> queryBackupStorageInSpecifyDomains(String domainIds) throws SQLException {
        StringBuilder domainIdsBuilder = new StringBuilder();
        if (domainIds.length() > 0) {
            String[] tmpArr = domainIds.split(",");
            for (String str : tmpArr) {
        ...
        return (List<BackupStorageEntity>) list("select DISTINCT(tb_sr) "
                        + " from BackupStorageEntity tb_sr where tb_sr.domain.id in (" + domainIdsBuilder.toString()
                        + ")");
    }
     ...
     *             sql异常
     */
    public BackupStorageEntity obtainMaxBackupStorage() throws SQLException {
        return uniqueResult("select tb_bu1 from BackupStorageEntity tb_bu1 where tb_bu1.availableCapacity = (select max(tb_bu2.availableCapacity) from BackupStorageEntity tb_bu2 )");
    }
}
