...
(function() {
    ...
    GuacamoleUI.buttons.showClipboard.onclick = function() {
        ...
        if (displayed != "block") {
            ...
            GuacamoleUI.buttons.showClipboard.innerHTML = "隐藏剪贴板";
        }
        else {
            GuacamoleUI.containers.clipboard.style.display = "none";
            GuacamoleUI.buttons.showClipboard.innerHTML = "显示剪贴板";
    ...
    GuacamoleUI.buttons.showKeyboard.onclick = function() {
        ...
        if (displayed == "block") {
            ...
            GuacamoleUI.buttons.showKeyboard.textContent = "显示键盘";
        ...
        // Otherwise, show it
            ...
            GuacamoleUI.buttons.showKeyboard.textContent = "隐藏键盘";
...
GuacamoleUI.attach = function(guac) {
    ...
    guac.onstatechange = function(clientState) {
        ...
        switch (clientState) {
            ...
            case 1:
                GuacamoleUI.shadeMenu();
                GuacamoleUI.showStatus("连接中...请稍等!");
                title_prefix = "[连接中...]";
                break;

            // Connected + waiting
            case 2:
                GuacamoleUI.showStatus("已连接，等待渲染界面!");
                title_prefix = "[渲染界面...]";
                break;
            ...
            // Disconnecting
            case 4:
                GuacamoleUI.showStatus("断开连接中...");
                title_prefix = "[断开连接中...]";
                break;

            // Disconnected
            case 5:
                GuacamoleUI.showStatus("断开连接.");
                title_prefix = "[断开连接]";
                break;

            // Unknown status code
            default:
                GuacamoleUI.showStatus("[未知状态]");
    ...
    guac.onerror = function(error) {
        ...
        //xgj 
        if(error=="Error initializing VNC client"){
        	error="初始化失败：（1）出现这个的原因可能是您已经打开了一个连接，浏览器只支持一个远程连接，您可以选择客户端远程。（2）也有可能是因为端口不正确！"
        }
